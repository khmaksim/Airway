...
'Date 2020-09-08
'Aviacominfo.com
'Drawing on map Airway
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''

Const AA = 6378245.0
Const BB = 6356863.0
Const FileAdres = "D:\airway.txt"
...
Sub PlaceAirway(lat() As String, lon() As String, points() As String, nameAirway As String)
    ...
    MbeSendKeyIn "PLACE TEXT BELOW;"+nameAirway
    ...
    For i = LBound(lat) To UBound(lat) - 2
        ...
        MbeSendKeyIn "XY="+Str$((drX1 + drX2) / 2)+","+Str$((drY1 + drY2) / 2)+";"
    Next i
    MbeSendReset
    
    For i = LBound(lat) To UBound(lat) - 1
        ' Conver to radian
        drLatt1=LatToRad(ArincToRncLat(lat(i)))
        drLont1=LonToRad(ArincToRncLon(lon(i)))
        ...
        MbeSendKeyIn "ACTIVE LEVEL=17;"
        MbeSendKeyIn "ACTIVE FONT=ft-34;"  
        MbeSendKeyIn "PLACE TEXT;"+points(i)
        MbeSendKeyIn "XY="+Str$(drX1 + 5)+","+Str$(drY1)+";"
        MbeSendReset
    Next i
End Sub 
...
Sub Main
    ...
    Dim latP As String, lonP As String
    Dim nameAirway As String, namePoint As String
    Dim points() As String, lat() As String, lon() As String
    Dim i As Integer

    do
        Line Input #1, nameAirway

        i = 0
        do
            Line Input #1, namePoint

            'Check end file or split - 1
            If namePoint = "1" Then Exit Do
        ...
        PlaceAirway lat, lon, points, nameAirway 
    Loop while not EOF(1)
